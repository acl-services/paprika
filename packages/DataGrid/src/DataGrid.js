...
const DataGrid = React.memo(
  React.forwardRef((props, ref) => {
    ...
    const handleKeyDownGrid = event => {
      handleKeyDown({ data, ColumnDefinitions })(event);
    };

    const handleKeyUpGrid = event => {
    ...
    const handleItemsRendered = React.useCallback(
      ({ visibleRowStartIndex, visibleRowStopIndex }) => {
        ...
        if (InfiniteScroll) {
          ...
          console.log(`${visibleRowStopIndex + rowsOffset} > ${rowCount}`);
  ...
  (prev, next) => {
    if (prev.rerender !== null && prev.rerender === next.rerender) return true;
  }
);
