...
2. Make sure the `isSticky` columns are grouped together before the columns that are not sticky. This prevents issues with keyboard navigation.
...
There are two patterns to load more data shipped with this component.
1. Using a load more button
2. Infinite scrolling

Use the `<DataGrid.WhenScrollBarReachedBottom>` component which lets you know if the scrollbar reaches the bottom of the table viewport.
...
### Infinity Scroll
Use the `<DataGrid.InfinityScroll />` component to accomplish the infinity scroll pattern.
...
4. The browsers some are better than others
...
**Sticky headers and columns on my DataGris are not in sync while scrolling.**
