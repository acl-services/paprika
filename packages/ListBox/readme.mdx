...
- ListBoxWithTags `[←]` and `[→]` keys navigation, though you can use `[delete]` for remove the items.
...
- As soon the Listbox is open the user can start interacting with the `[↑]` and `[↓]` keys to navigate to different options.
...
### Single select with pre-built checkers

`wip` The listbox accepts a renderProp property which allowed the listbox to render its own checkers,
I think the checkbox is widely use that would make sense to have a prebuilt option for it, thought I'm
not sure if we should merge it with the renderChecker option, suggestion are need.

<Playground>
  <ListBox renderChecker="checkbox" zIndex={10000} getScrollContainer={() => document.querySelector("#root > div")}>
    <ListBox.Option>Spiderman</ListBox.Option>
    <ListBox.Option>Venom</ListBox.Option>
    <ListBox.Option>Carnage</ListBox.Option>
  </ListBox>
...
<ListBox renderChecker="checkbox">
  <ListBox.Option>Spiderman</ListBox.Option>
  <ListBox.Option>Venom</ListBox.Option>
  <ListBox.Option>Carnage</ListBox.Option>
</ListBox>
...
<Playground>
  <ListBox height={300} hasFilter zIndex={10000} getScrollContainer={() => document.querySelector("#root > div")}>
    {images.map(image => (
      <ListBox.Option label={image.label} key={image}>
        <ImageOption>
...
property behaves. While the single select always return the selected index plus the options.

The Multi ListBox will return an array of selected index, the options currently in the ListBox, plus
two more parameter, the index of the latest clicked item and if the item was added or removed from the
selection.
