...
<Toast hasCloseButton={false}>
  You can render whatever you need into the `renderActions` space not only a menu, can be a button, icon, etc. It will always be align to the right.
</Toast>
...
function App() {
  return (
    <Table data={data}>
      <Table.ColumnDefinition
        header={() => <DataHeader label="Value" type={DataHeader.type.types.TEXT} />}
        cell="name"
      />
      <Table.ColumnDefinition
        header={() => <DataHeader label="Value" type={DataHeader.type.types.NUMERIC} />}
        cell="name"
      />
      <Table.ColumnDefinition
        header={() => <DataHeader label="Value" type={DataHeader.type.types.DATA} />}
        cell="income"
      />
      <Table.ColumnDefinition
        header={() => <DataHeader label="Value" type={DataHeader.type.types.TIME} />}
        cell="taxes"
      />
      <Table.ColumnDefinition
        header={() => <DataHeader label="Value" type={DataHeader.type.types.DATE_TIME} />}
        cell="income"
      />
    </Table>
  );
