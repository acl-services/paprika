...
const propTypes = {
  ...
  cssOverrides: PropTypes.oneOf([PropTypes.string,PropTypes.func]),
...
const Button = React.forwardRef((props, ref) => {
  const { a11yText, canPropagate, children, cssOverrides, isDisabled, isSemantic, isSubmit, onClick, tabIndex, ...moreProps } = props;
  ...
  return isSemantic ? (
    <button css={`${buttonStyles} ${cssOverrides}`} {...buttonProps}>
      {children}
    </button>
  ) : (
    <RawButton css={`${buttonStyles} ${cssOverrides}`} {...buttonProps}>
      {children}
    </RawButton>
  );
});
