...
function Dialog(props) {
  ...
  const refPanels = React.useRef(null);
  ...
  const dialogMain = (
    <React.Fragment>
      <sc.DialogContent
        data-pka-anchor="panels.content"
        hasPushedElement={!!getPushContentElement}
        isCompact={isCompact}
        isOpen={isOpen}
        kind={kind}
        ref={refPanelsContent}
        role="region"
        tabIndex="0"
      >
        {children}
  ...
  const dialogFooter = footer ? React.cloneElement(footer, { refPanels, isCompact }) : null;
...
const propTypes = {
  ...
  refPanelsContent: PropTypes.shape({ current: PropTypes.instanceOf(Element) }).isRequired,
