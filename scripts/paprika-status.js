...
class PaprikaStatus {
  ...
  getCommitPrefixCounts(subDir, commits) {
    ...
    try {
      ...
      child_process.execSync(`git rev-list --count HEAD --grep="fix(" --count -- ${path} > ${path}/${this.bugCountOutput}`);
      child_process.execSync(`git rev-list --count HEAD --grep="chore(" --count -- ${path} > ${path}/${this.choreCount}`);
  ...
  getTestStatistics(subDir, commits) {
    try {
      const jestSummary = [];
      const output = child_process.execSync(`yarn jest --passWithNoTests --testPathPattern=${this.PATH_TO_COMPONENTS + subDir} --coverage --coverageReporters="json-summary" --forceExit`).toString();
  ...
  getPaprikaStatuses() {
    ...
    fs.readdirSync(this.PATH_TO_COMPONENTS).forEach(subDir => {
      ...
      try {
          commits.version = JSON.parse(fs.readFileSync(`${this.PATH_TO_COMPONENTS + subDir}/${this.version}`, 'utf8')).version;
